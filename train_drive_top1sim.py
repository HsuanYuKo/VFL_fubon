...
from preprocess.drive import load_both

now_string = datetime.now().strftime('%Y-%m-%d-%H-%M-%S')
os.chdir(sys.path[0] + "/../")  # change working directory
root = "data/drive/38cf/"
A_dataset = root + "Drive_Party_A.csv"
B_dataset = root + "Drive_Party_B.csv"
...
num_common_features = 38
[X1, X2], y = load_both(drive_path_A=A_dataset, drive_path_B=B_dataset, active_party='drive')
name = "drive_top1sim"
...
model = Top1SimModel(num_common_features=num_common_features,
                     ...
                     val_rate=0.1,
                     test_rate=0.1,
                     drop_key=False, #False代表把common feature放進資料一起練
                     device='cuda:{}'.format(args.gpu),
                     hidden_sizes=[600, 300, 100],
...
model.train_splitnn(X1, X2, y, data_cache_path="cache/drive.pkl", scale=True)
